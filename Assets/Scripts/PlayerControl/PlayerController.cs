 ...
 * 
 *  功能简述：
 *  PlayerController 负责玩家角色的基础移动与物理行为控制，
 *  包括位移、跳跃、重力、瞄准状态切换以及移动相关事件分发，
 *  是玩家行为的核心驱动模块。
 *
 *  主要功能：
 *  - 处理玩家移动输入并计算最终位移速度
 *  - 基于摄像机朝向进行移动方向投影
 *  - 管理跳跃、多段跳与落地判定
 *  - 自行实现重力逻辑（不依赖 Rigidbody）
 *  - 管理肩射 / 开镜瞄准状态切换
 *  - 对外分发移动、跳跃、落地、瞄准等事件
 *  - 支持技能期间对玩家移动的锁定与恢复
 *
 *  设计说明：
 *  - 输入通过 ILocomotionInputSource 接口注入，支持本地 / 网络 / AI 复用
 *  - 使用 CharacterController 实现角色运动，避免 Rigidbody 的性能与控制复杂度
 *  - 重力与跳跃逻辑完全由代码控制，便于技能扩展与差异化角色设计
 *  - 移动、跳跃、瞄准均采用事件驱动，解耦动画、相机与表现层
 *  - Update / LateUpdate 分离，用于区分物理计算与相机/朝向更新
 *
 *  使用说明：
 *  - 需在角色上挂载 CharacterController 组件
 *  - GroundLayers 用于地面检测，请正确配置
 *  - 外部系统（动画、相机、技能）应通过事件监听与本类交互
