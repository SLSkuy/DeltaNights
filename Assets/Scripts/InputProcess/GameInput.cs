 ...
 * 
 *  功能简述：
 *  GameInput 负责统一捕获并管理玩家输入数据，
 *  向移动、视角、攻击与技能系统提供标准化输入接口。
 *
 *  主要功能：
 *  - 管理角色移动、视角、攻击与技能相关输入
 *  - 作为 Cinemachine 输入轴的数据提供者
 *  - 通过接口对外暴露输入状态，便于系统解耦与扩展
 *
 *  使用说明：
 *  - 场景中仅允许存在一个 GameInput 实例
 *  - 输入参数通过 Inspector 进行配置
 *  - 其他系统通过接口读取输入，不直接访问具体输入轴
 * ------------------------------------------------------------ */
